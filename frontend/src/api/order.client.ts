...
export const orderClientPublic = {
    ...
    createRazorpayOrder: async (eventId: IdParam, orderShortId: IdParam) => {
        const response = await publicApi.post<RazorpayOrder>(`events/${eventId}/order/${orderShortId}/razorpay/order`);
        return response.data;
    },

    verifyRazorpayPayment: async (eventId: IdParam, orderShortId: IdParam, payload: RazorpayPaymentResponse) => {
        const response = await publicApi.post<GenericDataResponse<RazorpayPaymentVerification>>(`events/${eventId}/order/${orderShortId}/razorpay/verify`, payload);
        return response.data;
    },
}
