...
class UpdateEventSettingsRequest extends BaseRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // Payment settings
            'payment_providers' => ['array', function ($attribute, $value, $fail) {
                if (is_array($value) && in_array(PaymentProviders::RAZORPAY->value, $value)) {
                    $this->validateRazorpayConfiguration($fail);
                }
            }],
    ...
    public function messages(): array
    {
        ...
        return [
            ...
            // Razorpay messages
            'payment_providers.razorpay_configuration' => __('Razorpay configuration is incomplete. Please ensure RAZORPAY_KEY_ID, RAZORPAY_KEY_SECRET, and RAZORPAY_WEBHOOK_SECRET are configured.'),
        ];
    }

    /**
     * Validate Razorpay configuration when Razorpay is selected as a payment provider
     */
    private function validateRazorpayConfiguration($fail): void
    {
        $requiredConfig = [
            'services.razorpay.key_id' => 'RAZORPAY_KEY_ID',
            'services.razorpay.key_secret' => 'RAZORPAY_KEY_SECRET',
            'services.razorpay.webhook_secret' => 'RAZORPAY_WEBHOOK_SECRET',
        ];

        $missingConfig = [];

        foreach ($requiredConfig as $configKey => $envVar) {
        ...
        if (!empty($missingConfig)) {
            $fail(__('Razorpay configuration is incomplete. Missing: :missing', [
